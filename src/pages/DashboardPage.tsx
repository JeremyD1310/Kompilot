...
function ActionSection({ title, description, actions, emptyLabel, onPrimary, onRemove }: { title: string; description: string; actions: CockpitAction[]; emptyLabel: string; onPrimary: (action: CockpitAction) => void; onRemove: (id: string, label: string) => void }) {
  return (
    <section className={`${cardClass} p-4 sm:p-5`}>
      ...
      <div className="space-y-2">{actions.length ? actions.map(action => <ActionRow key={action.id} action={action} onPrimary={() => onPrimary(action)} onSnooze={() => onRemove(action.id, 'reportée')} onIgnore={() => onRemove(action.id, 'ignorée')} />) : <div className="rounded-xl border border-dashed border-border p-5 text-center text-sm text-muted-foreground">{emptyLabel}</div>}</div>
    </section>
  );
}
...
export default function DashboardPage() {
  ...
  const removeAction = (id: string, label: string) => { setDismissed(previous => [...previous, id]); toast.success(`Action ${label}`, { description: 'Vous pourrez la retrouver dans son hub métier.' }); };
  ...
  const demoReviews = isDemoActive ? demo.demoData.reviews.filter(item => !item.replied) : [];
  ...
  const weekly = [
    { label: 'Publications préparées', value: isDemoActive ? demo.demoData.publications.length : allPosts.length, icon: PenLine },
    { label: 'Avis suivis', value: isDemoActive ? demo.demoData.reviews.length - demoReviews.length : Math.max(0, (activeEstablishment?.pendingReviews ?? 2) - pendingReviews), icon: Star },
    { label: 'Visibilité locale', value: isDemoActive ? demo.data.localVisibility : 78, icon: Target },
    { label: 'Actions validées', value: isDemoActive ? demo.data.approvals.filter(item => item.status === 'Validé').length : 0, icon: CheckCircle2 },
  ];
    ...
    <Page>
      ...
      <PageBody className="space-y-6 px-4 pb-12 pt-5 sm:px-6 sm:pt-7">
        ...
        <div id="priority-actions" className="grid gap-4 xl:grid-cols-2">
          ...
          <motion.div custom={1} variants={fadeUp} initial="hidden" animate="visible"><ActionSection title="Préparé par Kompilot" description="L’IA prépare, vous relisez avant toute diffusion." actions={prepared} emptyLabel="Aucune suggestion préparée." onPrimary={openAction} onRemove={removeAction} /></motion.div>
          <motion.div custom={2} variants={fadeUp} initial="hidden" animate="visible"><ActionSection title="En attente de validation" description="Rien n’est publié ou envoyé sans votre accord." actions={validation} emptyLabel="Aucune validation en attente." onPrimary={openAction} onRemove={removeAction} /></motion.div>
          <motion.section custom={3} variants={fadeUp} initial="hidden" animate="visible" className={`${cardClass} p-4 sm:p-5`}><div className="mb-4 flex items-start justify-between gap-3"><div><h2 className="text-base font-black text-foreground">Résultats de la semaine</h2><p className="mt-1 text-xs text-muted-foreground">Exemples issus de vos actions récentes.</p></div><Link to="/performance" className="text-xs font-bold text-primary hover:underline">Voir le rapport</Link></div><div className="grid grid-cols-2 gap-2 sm:grid-cols-4">{weekly.map(({ label, value, icon: Icon }) => <Link key={label} to="/performance" className="rounded-xl bg-secondary/60 p-3 transition hover:-translate-y-0.5 hover:bg-primary/10"><Icon size={16} className="text-primary" /><p className="mt-3 text-xl font-black text-foreground">{value}</p><p className="mt-1 text-[11px] leading-4 text-muted-foreground">{label}</p></Link>)}</div><div className="mt-4 flex flex-wrap gap-2"><Link to="/performance" className={`${touchClass} inline-flex items-center gap-2 border border-border text-foreground`}><BarChart3 size={15} /> Rapport détaillé</Link><button type="button" onClick={() => toast('Rapport prêt à être partagé', { description: 'L’envoi reste soumis à votre validation.' })} className={`${touchClass} inline-flex items-center gap-2 text-primary`}><FileText size={15} /> Préparer un rapport</button></div></motion.section>
        </div>
        ...
        {!isDemoActive && user && <OnboardingChecklist onConnectAccount={() => setConnectAccountOpen(true)} onCreatePost={() => openCreatePost()} />}
      </PageBody>
      ...
      <MilestoneCelebrationModal data={{ startRank: 14, currentRank: 8, joinedMonth: '2025-01', avgRating: 4.5, startReviews: 5, currentReviews: 18, copilotReplied: 14, primaryKeyword: `${activeEstablishment?.category ?? 'commerce'} ${activeEstablishment?.address ?? 'votre ville'}` }} />
