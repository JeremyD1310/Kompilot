...
// Unified key — must match DashboardLayout.tsx
const SIDEBAR_KEY = 'kompilot_sidebar_collapsed'
...
export function AppSidebarShell() {
  ...
  const [collapsed, setCollapsed] = useState(() => {
    ...
    // DashboardLayout stores '1'/'0'; support both formats
        ...
        >
          ...
          {collapsed ? (
            <Tooltip>
              ...
              <TooltipContent side="right">
                {user?.displayName || user?.email || 'Utilisateur'} · {user?.email || ''}
              </TooltipContent>
            </Tooltip>
          ) : (
