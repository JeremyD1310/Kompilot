  ...
  const result = await blink.db.batch([
    {
      ...
      args: [
        ...
        description || `Consumed ${cost} credit(s) for ${actionType}`, safeReferenceId, now, creditType,
...
export async function consumeExecuteRefund<T>(
  ...
  const consumed = await consumeCredits(blink, userId, actionType, description, referenceId, creditType);
