...
import { blinkBackendUrl, liveWebhookSecret, liveStripeKey } from '../lib/liveBilling';
...
router.post('/api/webhooks/stripe', async (c) => {
  ...
  const webhookSecret = liveWebhookSecret(rawEnv) ?? undefined;
  ...
  try {
    ...
    if (prior[0]?.status === 'processed') return c.json({ received: true, replay: true, type: event.type });
    if (!prior.length) throw new Error('Unable to record Stripe event');
    // A prior processing attempt may have failed; continue so Stripe retries are effective.
    await blink.db.stripe_webhook_events.update(event.id, { status: 'processing', error: null } as any);
  }
  ...
  try {
    ...
    if (event.type === 'invoice.payment_failed') {
      ...
      if (userId) {
        ...
        try {
          const backendUrl = blinkBackendUrl(rawEnv);
          const secretKey  = (rawEnv as any).BLINK_SECRET_KEY as string | undefined;
          if (!backendUrl || !secretKey) throw new Error('Backend alert configuration is missing');
    ...
    if (event.type === 'customer.subscription.created') {
      ...
      if (userId) {
        ...
        const resolved  = await resolvePriceIdToPlan(stripeKey, priceId)
          ?? (firstItem?.price?.lookup_key ? resolvePriceToPlan(firstItem.price.lookup_key, rawEnv) : null);
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      if (userId && session.mode === 'subscription') {
        ...
        const resolved  = await resolvePriceIdToPlan(stripeKey, firstPriceId)
          ?? (firstLookupKey ? resolvePriceToPlan(firstLookupKey, rawEnv) : null);
        const finalPlanId  = resolved?.planId ?? planId;
        const finalBilling = resolved?.billing ?? billing;
      ...
      if (userId) {
        ...
        if (entitlements) {
          ...
          const grant = async (creditType: 'ai' | 'sms', amount: number) => {
            const grantId = `grant:${userId}:${creditType}:${periodKey}`.slice(0, 255);
            try {
        ...
        try {
          const trackingBackendUrl = blinkBackendUrl(rawEnv);
          if (!trackingBackendUrl) throw new Error('Backend tracking configuration is missing');
    ...
    console.error(`[webhook/stripe] Error processing event ${event.id} (${event.type}):`, error);
    await blink.db.stripe_webhook_events.update(event.id, { status: 'failed', error: (error as any).message || String(error) } as any);
    // Return a non-2xx response so Stripe retries the failed event.
    return c.json({ received: false, type: event.type, status: 'failed' }, 500);
  }
