...
router.post('/api/billing/checkout', async (c) => {
  ...
  const lookupKey = resolvedPlan.lookupKey;
  const pricesRes = await fetch(`https://api.stripe.com/v1/prices?${new URLSearchParams({ lookup_keys: lookupKey, active: 'true', limit: '1' })}`, {
    headers: { Authorization: `Bearer ${stripeKey}` },
  });
  if (!pricesRes.ok) return c.json({ error: 'Prix Stripe non configuré côté serveur', code: 'PRICE_NOT_CONFIGURED' }, 503);
  const prices = await pricesRes.json() as { data?: Array<{ id: string }> };
...
router.post('/api/billing/credit-pack', async (c) => {
  ...
  const label = product.definition.name;
  const amount = product.definition.amount / 100;
  ...
  const sessionParams = new URLSearchParams({
    ...
    'line_items[0][price_data][product_data][description]': `${credits} crédits ${product.definition.creditType ?? 'IA'} pour Kompilot`,
    'line_items[0][price_data][unit_amount]': String(product.definition.amount),
    'line_items[0][quantity]': '1',
